...
export const DocumentationPage: React.FC = () => {
    ...
    const filteredTasks = useMemo(() => {
        ...
        return allTasks.filter(item => {
            ...
            if (activeFlow === 'to_print') {
                ...
                // CORREÇÃO DA LÓGICA: Um item está "em campo" se está preparado (impresso) E ainda não foi executado.
                if (!isPrepared || status === MaintenanceStatus.Executed) return false;
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-20">
                            ...
                            {activeFlow === 'to_print' && (
                                <>
                                    ...
                                    <button onClick={handleManualMove} className="px-4 py-2 bg-slate-200 text-slate-600 font-black text-xs uppercase rounded-lg hover:bg-slate-300">
                                        Marcar como Impresso (Manual)
                                    </button>
                                </>
                            )}
                            {activeFlow === 'in_field' && (
                                <button onClick={() => setIsBulkPrintOpen(true)} className="px-4 py-2 bg-blue-600 text-white font-black text-xs uppercase rounded-lg shadow-md hover:bg-blue-700 flex items-center gap-2">
                                    <RefreshIcon className="w-4 h-4"/> Reimprimir ({selectedTaskKeys.size})
                                </button>
                            )}
                        </>
                    )}
            ...
            {isBulkPrintOpen && <BulkPrintModal isOpen={isBulkPrintOpen} onClose={() => setIsBulkPrintOpen(false)} tasks={filteredTasks.filter(t => selectedTaskKeys.has(t.key))} documentType={selectedType} isReprint={activeFlow === 'in_field'} />}
            {individualPrintTask && <PreviewWorkOrderModal isOpen={!!individualPrintTask} onClose={() => setIndividualPrintTask(null)} taskData={individualPrintTask} isReprint={activeFlow === 'in_field'} />}
        </div>
    );
};
